...
const fetchHighlights = async (abbreviation: string) => {
    try {
      ...
      const res = await fetch(`/api/highlights?abbreviation=${abbreviation}`); // Use the countryAbbreviation here
      console.log("the data in fetchHighlights",abbreviation);
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    const getLocationAndFetchHighlights = async () => {
      try {
        await fetch("https://ipinfo.io?token=e22317b476e9fb");
